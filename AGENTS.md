...
- Target cross-service APIs, scoped Agent tasks, browser operation tools, snapshot-first events, idempotency and recovery are fixed in `ai-e2e/docs/service-api-event-contract.md`; dual-model and declarative Skill rules are fixed in `ai-e2e/docs/ai-model-skill-contract.md`. Proxy session/lease/operation control core, screenshot/DOM capture and browser events, plus ai-chat-service Agent task commands/events/Skill catalog/runtime, are shipped; ai-e2e Authoring/Run APIs, cross-service orchestration, general vision v2 and complete authorization remain pending.
