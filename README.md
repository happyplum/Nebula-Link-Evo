...
**状态、决策与证据（shipped）**：Run/计划/TODO/依赖/页面任务/尝试/变量/决策/命令/事件、执行协调、依赖传播、决策应用、公开 Run API/SSE 和 proxy operation/截图/DOM 证据提升已交付。登出中断、前置阻塞、待决策、依赖跳过、取消和业务失败保持独立语义；失败证据缺失只降低完整度。证据自动脱敏和保留清理仍为技术债。完整契约见 [`ai-e2e/docs/run-state-decision-evidence-contract.md`](ai-e2e/docs/run-state-decision-evidence-contract.md)。
