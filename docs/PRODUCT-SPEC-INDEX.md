...
| `agent-activity-ui`                    | —（UI 库）      | 统一 Agent 活动 reducer 与 React renderer | [`agent-activity-ui/PRODUCT-SPEC.md`](../agent-activity-ui/PRODUCT-SPEC.md)                                       | 根 `AGENTS.md`                                              |
...
| Chat Agent Stream               | `ai-chat-service`                   | `debug-ui`                                              | `GET /api/v1/chat/sessions/:sessionId/stream`                                                                                         | 仅发送 `agent_stream.snapshot` → `agent_stream.event`；可见历史与 live 同源，公开消息历史 GET 不存在                                                                                                                                                                                                                                                                                                                           |
| Chat sessions                   | `ai-chat-service`                   | `debug-ui`                                              | `* /api/v1/chat/sessions`                                                                                                            | 会话 CRUD                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Chat control                    | `ai-chat-service`                   | `debug-ui`                                              | `POST /api/v1/chat/sessions/:sessionId/{pause,resume,interrupt,cancel}`                                                              | 暂停/恢复/中断/取消                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| 受限 Agent task（shipped）      | `ai-chat-service`                   | `ai-e2e`                                                | `POST/GET /api/v1/agent-tasks*`、`POST /:taskId/commands`、`GET /:taskId/{events,event-log,activity,activity-log}`、`GET /api/v1/skills` | `/events` 是控制面审计；`/activity` 是脱敏呈现流，两者序号和职责独立。模型只提交冻结 stepId，target/args 与 policy/grant/effect/数量/lease 由 wrapper 注入                                                                                                                                                                                                          |
...
| E2E Agent 活动（shipped）       | `ai-e2e`                            | `ai-e2e ui`                                             | `GET /api/v1/{authoring-jobs/:jobId,runs/:runId}/{activity,activity-log}`                                                            | additive 本地单调序列；独立 activity cursor 聚合 Agent Task 活动并投影审批/验证/激活/依赖跳过等控制事实；按业务上下文隔离                                                                                                                                                                                                                                                                                                      |
...
| 浏览器执行控制面（in-progress） | `proxy-adapter`                     | `ai-e2e` / `ai-chat-service` / `browser-control-client` | `/api/v1/browser-execution/*`                                                                                                        | session/lease/operation query、artifact、snapshot-first SSE/event-log、TTL/hold 原始产物清理与 operation/idempotency 7 天账本清理 worker 已交付；CLI/Harness 客户端不增加路由。单活动 session/单 Context、opaque token hash、SQLite WAL、直接调试访问仲裁和重启 `outcome_unknown` 已生效；v1 在安全边界撤销/重发 control lease，不提供原地续租；未处理证据保持 `restricted/pending`，不承诺通用自动脱敏；video 和动画仍 pending |
...
- shared 定义唯一 `AgentStreamSnapshotV1/EventV1/TurnV1/SectionV1`；所有事件带稳定 stream/turn/section 身份、单调 seq 和时间。
...
- Agent 活动消费：通过 `/agent-tasks/:taskId/activity-log` 使用独立 cursor 聚合，并与 authoring/run 控制事实投影到各自 `/activity`；跨项目、URL 或业务上下文不得返回不可见活动。
- v1 三服务新控制面只允许 loopback/local 单用户部署；proxy 与 ai-chat 的可配置启动入口拒绝非 loopback host，ai-e2e 固定监听 `127.0.0.1`。capability、Origin 和 lease 不替代认证；远程/多用户属于产品范围扩展，在启用前必须另行定义并交付统一身份、授权与租户隔离，但不是当前 v1 技术债。
- browser lease 使用短期 32-byte opaque token，proxy 只持久化 hash/policy/expiry/process epoch；operation ledger 默认使用 proxy 自有 SQLite WAL。observe 默认最多 30 秒/一次指定观测，control 默认最多 5 分钟；客户端只在原子操作安全边界撤销并重发租约，v1 不提供原地续租。
...
- **决策与证据（in-progress）**：版本/运行决策、计划级 grant、内容寻址 artifact、append-only evidence、sealed manifest、proxy operation/截图/DOM 自动提升、proxy 短期原始产物 TTL/hold 清理及 ai-e2e 长期原始证据 7/30 天保留清理已交付；未按项目规则处理的截图/DOM 保持 `restricted/pending`。v1 不承诺通用自动脱敏；开放证据外发、共享、远程/多用户访问或项目级隐私策略前，必须先定义脱敏与权限验收标准并交付实现。生产 UI 恢复仍 pending。
...
| 修改 Agent 活动契约、投影或渲染行为                                                        | 跨包契约（3.2、3.4、3.5、3.7） + `shared`、`agent-activity-ui`、`debug-ui`、`ai-chat-service`、`ai-e2e` PRODUCT-SPEC + 服务事件契约与 shipped 清单                                                                                                                                                                                     |
