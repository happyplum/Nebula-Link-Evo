...
- [designed] v1 capability 固定 `maxActiveBrowserSessions=1`；业务 FIFO 由 ai-e2e 持有，proxy 只做不解释业务类型的通用独占门禁。最多一个 control lease，observe 只在操作安全边界，live view 无控制权；受控 session 期间 legacy MCP/debug 写工具返回 `browser_busy`。
