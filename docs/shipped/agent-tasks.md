...
- [shipped] `GET /api/v1/capabilities` 声明 agent-task/browser-operation `1.0`、已实现功能和硬限制；`taskEvents/taskCommands=true`，`skillsRuntime/operationPresentationAnimation=false`；capability 可读，Agent task 控制面要求 ai-chat-service 绑定 loopback。
...
- [pending] Skills loader/runtime、通用视觉 v2 与完整 policy evaluation/风险投影/active grant/参数级数量交集仍未实现；capability 只对这些未实现能力保持 false。
