...
- [shipped] LiveKit 首屏按需边界：新用户默认 MJPEG，既有持久化传输选择继续生效；只有选择 WebRTC 才动态加载 `LiveKitView` 与 `livekit-client`，加载或 chunk 失败期间保留 MJPEG。生产 HTML 不预加载 LiveKit，`page-load.spec.ts` 验证首屏零 LiveKit 请求并在点击后加载。
