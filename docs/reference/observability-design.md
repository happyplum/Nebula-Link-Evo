...
- `GET /api/v1/health` returns MCP inventory and proxy-adapter in-process browser engine liveness.
...
- Split health checks into:
  - `GET /api/v1/health/live` for process liveness only.
  - `GET /api/v1/health/ready` for readiness checks such as database and MCP readiness.
  - Keep `GET /api/v1/health` as the human-readable full status endpoint.
- Add `GET /api/v1/health/details` for debugging details such as uptime, memory, active sessions, queue depth, and dependency status.
...
Phase A must cut over capability and all callers atomically; it must not add old-path aliases or a compatibility layer and requires no new runtime dependencies.
...
- Expose `GET /api/v1/metrics` only when metrics are enabled.
...
- Should `/api/v1/metrics` be protected behind a reverse proxy rule or served on a separate admin port?
