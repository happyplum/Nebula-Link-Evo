...
| `/api/v1/browser-execution/sessions/:sessionId/events`、`event-log`、`/artifacts/:artifactId` | GET/SSE | pending | snapshot-first 会话事件、持久事件查询与短期原始产物读取 | 尚未交付；当前 capability 明确 `operationCaptureArtifacts=false` |
