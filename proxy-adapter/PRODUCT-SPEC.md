...
- v1 不承诺通用自动脱敏；未按上层项目规则处理的截图/DOM 必须保持受限状态。开放证据外发、共享、远程/多用户访问或项目级隐私策略前，调用方必须先定义可验证的脱敏与权限规则。
- control lease 单次最长 5 分钟，客户端仅可在原子操作安全边界撤销并重发；v1 不提供原地续租，只有运行指标证明轮换影响任务后才可另行定义续租协议。
...
| 模块             | 路径                                                                                                                                                                                                                 | 状态    | 职责                                                                                                                                                      | 边界/契约                                                                                                                                                                                      |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 服务入口         | `src/server.ts`                                                                                                                                                                                                      | shipped | Env 加载、显式路由注册、短期产物保留清理 worker 与启动                                                                                                    | 本进程唯一 dotenv owner；工作目录 `.env` → 父目录 `.env`，既有进程变量优先；随后执行 DB backup init（非测试）→ browser-execution service/产物清理/provider → canonical HTTP/MCP/debug surfaces |
| 应用服务         | `src/services/app-service.ts`                                                                                                                                                                                        | shipped | 浏览器状态与工具 inventory 单例 facade                                                                                                                    | 其他模块通过 AppService 访问能力                                                                                                                                                               |
| 交互日志         | `src/services/interaction-logger.ts`                                                                                                                                                                                 | shipped | 记录浏览器交互历史                                                                                                                                          | 写入本地 DB                                                                                                                                                                                    |
...
| 浏览器引擎       | `src/browser-engine/`（services/{browser-lifecycle,browser-service,dom-extractor,page-actions,click-resolution,browser-lock} / screencast / locator-generator / marker-injector / dom-utils / index） | shipped | 进程内 Playwright Chromium 控制、可选远程调试端口、页面 CDP 会话、DOM 提取、点击解析、视觉标记注入、屏播                                        | 当前自行启动 Chromium，不存在外部 `playwright-server` 或 `connectOverCDP` 连接链；7 级目标链：nebula-id → role → testid → aria → text → css → xpath                                            |
...
| 类型             | `src/types.ts`、`src/types/`（fastify.d / node-sqlite.d）与 `src/browser-client.ts`                                                                                                                                   | shipped | 包内共享类型与外部 .d 补充                                                                                                                                |                                                                                                                                                                                                |
...
| `/api/v1/capabilities`                                                                       | GET                                 | shipped | 声明 browser-execution/operation `1.0`、动作/观测、持久账本、画面能力与 session/Context 限制              | `maxActiveBrowserSessions=1`、`maxBrowserContextsPerSession=1`、不支持 storage-state 切换；生产 `start()` 对任何非 loopback `HOST` 启动失败，运行实例的 `localControlPlane=true` |
...
| 功能                               | 入口                                                                                                  | 状态        | 验收面                                                                  | 关联模块                                                                                                                                                                                                                                                                                                                                                                  |
| ---------------------------------- | ----------------------------------------------------------------------------------------------------- | ----------- | ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 受控浏览器观测/操作                | browser-execution/playwright-browser、validation                                                      | shipped     | 单元测试 + 集成测试                                                     | 仅 capability 声明的 observe/act 白名单；公共请求按 kind/operation 判别到精确 args，执行分支直接使用收窄参数；禁止任意 JS/CDP/裸坐标                                                                                                                                                                                                                                      |
...
| 视觉标记系统（Vision Marker）      | browser-engine/marker-injector、locator-generator                                                     | shipped     | marker-mode-e2e + 集成测试                                              | browser-engine、shared/types/vision-marker；Debug marker 的 type/value/dispatch 操作缺少 `param` 时在浏览器访问前拒绝                                                                                                                                                                                                                                                     |
...
| 通用浏览器执行会话与操作账本       | `src/browser-execution/`                                                                              | shipped     | service/repository 单元测试 + 真实 HTTP/MCP/Chromium + 跨进程重启恢复   | 全局单 session/单 Context、lease token hash/process epoch、FIFO 原子边界、幂等冲突、queued cancel、`outcome_unknown`、脱敏请求账本和 debug 直连仲裁；终态关闭 session 的 resolved operation 与 session/lease 幂等键默认 7 天后清理，未知结果、活动资源及未完成 artifact 保留；不解释 E2E actor/environment/审批                                                           |
