...
- v1 exposes at most one active application browser execution session per proxy process. Upstream owns business-job ordering; this package only enforces a generic exclusive admission gate and treats owner/correlation tags as opaque. One `control` lease may act, `observe` is admitted only at atomic-operation safe boundaries, and live-view consumers have no lease or control authority.
