...
export function up(db: DatabaseSync): void {
  ...
  db.exec(`
    CREATE INDEX IF NOT EXISTS idx_sessions_state_status ON sessions_state(status)
  `);

  db.exec(`
    CREATE INDEX IF NOT EXISTS idx_sessions_state_last_active ON sessions_state(last_active_at)
  `);
}
