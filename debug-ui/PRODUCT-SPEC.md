...
| Vite config                | `vite.config.ts`                                                                                                                                                                                                                | shipped | base `/debug/`；`/api/v1/{chat,ai,test-ai,config}` → :3001；其余 `/api`、`/debug/*`、`/mcp` → :3000 | canonical v1 AI 路径必须排在通用 `/api` proxy 之前                                       |
| 类型补充                   | `src/vite-env.d.ts`                                                                                                                                                                                                             | shipped | Vite 环境类型                                                                                       |                                                                                          |
| E2E                        | `e2e/`（specs、fixtures、utils）                                                                                                                                                                                                | shipped | Playwright shell/navigation + `/debug/api/stream` SSE 连接、失败回退与重连测试                      |                                                                                          |
