...
const explorationRoutes: FastifyPluginAsyncTypebox<ExplorationRouteOptions> = async (
  fastify,
  options
) => {
  ...
  function requireRuntimeClient(): AiE2eRuntimeClient {
    if (!runtimeClient) {
      throw ServiceError.unavailable('AI or browser gateway service is not configured');
    }
...
export default fp(explorationRoutes, {
  fastify: '5.x',
  name: 'exploration-routes',
  encapsulate: true,
});
