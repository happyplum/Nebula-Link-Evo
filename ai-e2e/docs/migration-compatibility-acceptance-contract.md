...
> 状态：已确认目标设计；安全审批策略与多账号/多角色范围仍需产品确认。
> 更新时间：2026-08-12。
...
1. **环境与副作用审批**：建议 v1 对 local/test 自动执行已声明副作用；staging 对删除、批量和不可逆操作在 run 开始前做一次计划级审批；production 默认只读，变更动作全部禁用。是否接受该默认策略？
2. **多账号/多角色**：建议 v1 一个 run 只有一个 BrowserContext 和一个活动身份；跨角色流程通过显式退出/登录脚本串行切换，不支持同时保持多个登录态。是否首期必须支持多 Context/并存身份？
