...
- 完整参数 Schema/页面匹配与基线采集仍为技术债；proxy TTL/hold 短期原始产物清理、ai-e2e 长期证据 7/30 天保留清理、功能脚本独立 Schema validator 与 Agent/browser 持久 event-log 游标消费已交付。未按项目规则处理的证据保持 `restricted/pending`；通用自动脱敏不是 v1 承诺，须在证据外发、共享、远程/多用户访问或项目级隐私策略启用前先定义验收标准。
