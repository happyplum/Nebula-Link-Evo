...
- **页面锚点（pending）**：以规范化 URL（含 path/hash route）+ 路由/查询参数集合识别页面目标；当前 `urls.url` 只保存完整 URL，尚无独立参数模型。
...
- **功能脚本与场景（pending）**：功能脚本是最小复用/修复单元；场景是业务验收单位，可跨模块、跨页面按顺序、依赖、重复和输入输出关系调用多个功能脚本。当前存储和版本仍以单个 scenario 对应 TypeScript script 为单位。
- **业务版本（pending）**：由用户创建，记录来源版本及可选部署/Git 标识；`copy` 深复制 PRD、决策、页面、模块、脚本、场景、TODO、DOM/定位基线和参考截图，且不复制运行历史、实际测试数据、凭据或临时变量。
...
- **可视语义执行（pending）**：权威资产是结构化语义功能脚本，所有浏览器动作通过 proxy-adapter 执行并关联实时画面、语义步骤和结果证据；当前 `npx tsx` 子进程执行器是待替换的现状，不是目标执行路径。
